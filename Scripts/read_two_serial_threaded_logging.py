...
if __name__=='__main__':
    ...
    try:
        while True:
            ...
            if new_data_arduino == False:
                vals_ard = arduinoSerial.read_from_buffer(1)
                if vals_ard is not None:
                    #print(vals_ard)
                    new_data_arduino = True
                
            if new_data_emlid == True and new_data_arduino == True:
                #print("filter buffer: ", filter_buffer)
                #print("emlid: ", vals_eml)
                print("arduino: ", vals_ard)

                if len(filter_buffer) < FILTER_WIDTH:
                    filter_buffer.append(vals_ard)
                    if len(filter_buffer) == FILTER_WIDTH:
                        print("buffer: ", filter_buffer)
                        filter_value = median([x[0][0] for x in filter_buffer])
                        #print("median: ", filter_value)
                        filter_buffer.pop(0)
                #for i in range(0, med_filt_len
                #print(filter_value)
                print("median: ", filter_value)
                new_data_emlid = False
                new_data_arduino = False
