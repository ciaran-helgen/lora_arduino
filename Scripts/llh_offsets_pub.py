...
def gps_callback(data):
    global gps_lat = data.latitude
    global gps_lon = data.longitude
...
def imu_callback(data):
    ...
    imu_quat = data.orientation # Type geometry_msgs/Quaternion
    (_,_,imu_yaw) = euler_from_quaternion([imu_quat.x, imu_quat.y, imu_quat.z, imu_quat.w])
    global heading_rel_north = imu_yaw - 90
...
def main():
    ...
    offset_pub = rospy.Publisher('position_offsets', Point32, queue_size=10)
    llh_pub = rospy.Publisher('lon_lat_offsets', NavSatFix, queue_size=10)
...
##                    gnd_truth_dist = calc_displacement(coord_from_emlid(vals_eml), gnd_truth_base_pos)
                ...
                # log the number of times ground truth was preferred. Throttle to print every 10 seconds
                rospy.loginfo_throttle(10, "replaced {} distance estimates".format(replacement_count))
                ...
                llh_offset_message = generate_llh_offsets(dispEst) # generates LLH offset NavSatFix message
